...
async fn stale_online_agent_for_same_role_is_buffered_when_task_session_does_not_match() {
    ...
    let msg = BridgeMessage {
        ...
        target: MessageTarget::Role { role: "lead".into() },
        reply_target: None,
        message: "route only to the current task".into(),
        timestamp: 1,
        reply_to: None,
        priority: None,
        status: None,
...
async fn agent_id_routing_broadcast_delivers_to_both_providers_for_same_role() {
    ...
    let msg = BridgeMessage {
        ...
        target: MessageTarget::Role { role: "coder".into() },
        reply_target: None,
        message: "implement feature X".into(),
        timestamp: 1,
        reply_to: None,
        priority: None,
        status: None,
...
async fn agent_targeted_delivers_to_one_agent_not_all_same_role() {
    ...
    let msg = BridgeMessage {
        ...
        reply_target: None,
        message: "direct to codex lead".into(),
        timestamp: 1,
        reply_to: None,
        priority: None,
...
async fn role_targeted_still_broadcasts_after_agent_routing_added() {
    ...
    let msg = BridgeMessage {
        ...
        target: MessageTarget::Role { role: "coder".into() },
        reply_target: None,
        message: "role broadcast to coder".into(),
        timestamp: 1,
        reply_to: None,
        priority: None,
        status: None,
...
async fn reply_target_explicit_agent_override_bypasses_redirect() {
    ...
    let explicit = BridgeMessage {
        ...
        reply_target: None,
        message: "to lead_2 directly".into(),
        timestamp: 2,
        reply_to: None,
        priority: None,
        status: Some(MessageStatus::Done),
...
async fn mixed_provider_guard_coder_to_lead_delivered() {
    // `reply_target_map` is a process-wide static; other tests that record
    // delegations (e.g. `reply_target_two_leads_separate_coder_chains`)
    // can leak entries keyed by "codex" into this test and cause the
    // role-targeted `lead` send to redirect to an offline-looking agent_id,
    // dropping delivery. Clear it at entry so this test is self-contained.
    clear_reply_targets();
