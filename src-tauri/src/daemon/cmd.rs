...
pub enum DaemonCmd {
    ...
    RespondPermission { request_id: String, behavior: PermissionBehavior },
    ForceDisconnectAgent { agent_id: String },
    // ── Task management ───────────────────────────────────────
    CreateTask { workspace: String, title: String, reply: oneshot::Sender<Task> },
    ListTasks { workspace: Option<String>, reply: oneshot::Sender<Vec<Task>> },
    SelectTask { task_id: String, reply: oneshot::Sender<Result<(), String>> },
    GetTaskSnapshot { reply: oneshot::Sender<Option<TaskSnapshot>> },
    ApproveReview { reply: oneshot::Sender<Result<(), String>> },
    ListSessionTree { task_id: String, reply: oneshot::Sender<Option<SessionTreeSnapshot>> },
    ListHistory { workspace: Option<String>, reply: oneshot::Sender<Vec<HistoryEntry>> },
    ResumeSession { session_id: String, reply: oneshot::Sender<Result<(), String>> },
}
