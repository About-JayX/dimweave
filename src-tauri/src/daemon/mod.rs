    ...
    while let Some(cmd) = cmd_rx.recv().await {
        match cmd {
            ...
            DaemonCmd::GetTelegramState { reply } => {
                let tg_state = telegram_get_runtime_state(&state, telegram_handle.is_some()).await;
            ...
            DaemonCmd::SaveTelegramConfig {
                ...
                let result = telegram_save_and_restart(
                    &state,
                    &app,
                    &mut telegram_handle,
                    bot_token,
            ...
            DaemonCmd::GenerateTelegramPairCode { reply } => {
                let result = telegram_generate_pair(&state, &app, telegram_handle.is_some()).await;
                let _ = reply.send(result);
            }
            DaemonCmd::ClearTelegramPairing { reply } => {
                let result = telegram_clear_pair(&state, &app, telegram_handle.is_some()).await;
                let _ = reply.send(result);
            }
        }
    }
...
async fn telegram_get_runtime_state(
    _state: &SharedState,
    connected: bool,
...
async fn telegram_save_and_restart(
    state: &SharedState,
    app: &AppHandle,
    ...
    let rs = telegram_get_runtime_state(state, handle.is_some()).await;
    gui::emit_telegram_state(app, &rs);
    Ok(rs)
}

async fn telegram_generate_pair(
    _state: &SharedState,
    app: &AppHandle,
    connected: bool,
) -> Result<crate::telegram::types::TelegramRuntimeState, String> {
