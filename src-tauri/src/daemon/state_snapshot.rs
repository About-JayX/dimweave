...
impl DaemonState {
    ...
    pub fn task_snapshot(&self) -> Option<TaskSnapshot> {
        ...
        let sessions: Vec<_> = self.task_graph.sessions_for_task(task_id)
            .into_iter().cloned().collect();
        let artifacts: Vec<_> = self.task_graph.artifacts_for_task(task_id)
            .into_iter().cloned().collect();
        Some(TaskSnapshot { task, sessions, artifacts })
    }

    /// List tasks, optionally filtered by workspace.
    pub fn task_list(&self, workspace: Option<&str>) -> Vec<Task> {
        match workspace {
            Some(ws) => self.task_graph.tasks_for_workspace(ws).into_iter().cloned().collect(),
    ...
    pub fn session_tree(&self, task_id: &str) -> Option<SessionTreeSnapshot> {
        ...
        let sessions: Vec<_> = self.task_graph.sessions_for_task(task_id)
            .into_iter().cloned().collect();
        Some(SessionTreeSnapshot { task_id: task_id.to_string(), sessions })
    }
    ...
    pub fn task_history(&self, workspace: Option<&str>) -> Vec<HistoryEntry> {
        ...
        tasks.into_iter().map(|t| {
            ...
            HistoryEntry { task: t.clone(), session_count, artifact_count }
        }).collect()
    }
    ...
    pub fn resume_session(&mut self, session_id: &str) -> Result<String, String> {
        let sess = self.task_graph.get_session(session_id)
            .ok_or_else(|| format!("session not found: {session_id}"))?.clone();
        ...
        match sess.role {
            SessionRole::Lead => { self.task_graph.set_lead_session(&sess.task_id, session_id); }
            SessionRole::Coder => { self.task_graph.set_coder_session(&sess.task_id, session_id); }
        }
        self.task_graph.update_session_status(session_id, SessionStatus::Active);
        self.auto_save_task_graph();
        Ok(sess.task_id)
    }
}
