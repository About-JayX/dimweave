...
| Task 3 | `c24c7025` | Closed spec to Accepted; added live-fix regression test proving card pills render in persisted order after role constraint and live Codex model wiring. | `bun test src/components/TaskPanel/TaskHeader.test.tsx` ✅ 20 passed; `git diff --check` ✅ | accepted |
