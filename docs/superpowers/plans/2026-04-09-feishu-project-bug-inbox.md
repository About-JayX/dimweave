...
- Modify: `src-tauri/src/daemon/control/mod.rs`
...
| Task 3 | `feat: accept feishu project webhooks into inbox` | `cargo test --manifest-path src-tauri/Cargo.toml feishu_project_webhook`; `cargo test --manifest-path src-tauri/Cargo.toml`; `git diff --check` | `f4cf1520` — Webhook is an optional fast path and validates the configured token, supports Feishu URL verification challenges, dedupes on `header.uuid`, and returns quickly. |
| Task 4 | `feat: add bug inbox shell panel` | `bun test src/components/ShellContextBar.test.tsx src/components/TaskContextPopover.test.tsx src/components/BugInboxPanel/index.test.tsx src/stores/feishu-project-store.test.ts`; `bun run build`; `git diff --check` | `de6fcb99` — The Bug Inbox now appears as a native shell tool with drawer rendering, item-count badge, configurable sync parameters, and a frontend store/event bridge for state + list updates. |
| Task 5 | `feat: launch linked dimweave tasks from bug inbox` | `cargo test --manifest-path src-tauri/Cargo.toml feishu_project::task_link`; `cargo test --manifest-path src-tauri/Cargo.toml`; `bun run build`; `git diff --check` | `9d7af78a` — `Start handling` is idempotent: it reopens linked tasks when present, recovers from stale links by relinking, writes a persisted snapshot file, and routes a Feishu-sourced handoff to lead. |
