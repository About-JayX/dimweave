...
| Task 4 | `docs: remove stale review gate references` | `manual diff review` | `rg -n "reviewer|ReviewStatus|review_gate" src-tauri/src bridge/src src/components src/stores CLAUDE.md .claude/rules -g'*.rs' -g'*.ts' -g'*.tsx' -g'*.md'`; `cargo test --manifest-path src-tauri/Cargo.toml`; `cargo test --manifest-path bridge/Cargo.toml`; `bun run build`; `git diff --check` | Final acceptance required one last docs-only sweep: the runtime was already clean, but `CLAUDE.md` still carried four stale `review gate` descriptions that needed to be removed so Source of Truth matched the implementation. |
