...
| Task 6 | `ad5230a5` | Mark singleton daemon role/connection fields as compatibility-only, add focused `runtime_final_cleanup` regressions, and document the final task-local ownership model. | `cargo test --manifest-path src-tauri/Cargo.toml runtime_final_cleanup -- --nocapture` ✅ 4 passed; `cargo test --manifest-path src-tauri/Cargo.toml routing_ -- --nocapture` ✅ 22 passed; `cargo test --manifest-path src-tauri/Cargo.toml codex:: -- --nocapture` ✅ 52 passed; `cargo test --manifest-path src-tauri/Cargo.toml claude_sdk:: -- --nocapture` ✅ 38 passed; `cargo test --manifest-path src-tauri/Cargo.toml daemon::provider::claude_tests:: -- --nocapture` ✅ 17 passed; `cargo test --manifest-path src-tauri/Cargo.toml daemon::provider::codex_tests:: -- --nocapture` ✅ 18 passed; `bun run build` ✅; `git diff --check` ✅ | accepted |
