...
| Task 4 | _(this commit)_ | Final regression test proving active-task switch syncs reply target via `selectActiveReplyTarget`; spec marked as implemented; CM record completed with all accepted commits and verification evidence. | `bun test src/components/ReplyInput/index.test.tsx` ✅ 7 passed; `git diff --check` ✅ | pending |
