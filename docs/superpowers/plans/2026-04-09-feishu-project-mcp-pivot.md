...
| Task 4 | `refactor: retire legacy feishu token sync path` | `cargo test --manifest-path src-tauri/Cargo.toml`; `bun run build`; `git diff --check` | `daef8f28` — Webhook handler + route deleted. `api.rs` deprecated in-place (serde compat). Legacy config fields marked deprecated. Legacy runtime state fields (`project_key`, `local_webhook_path`, `webhook_enabled`) removed from `FeishuProjectRuntimeState`. |
