...
| Task 2 | `feat: wire composer paste into attachment flow` | `bun test src/components/ReplyInput/paste-attachments.test.ts src/components/ReplyInput/index.test.tsx`; `bun run build`; `git diff --check` | `6a917d48` — Paste should be a third attachment source, not a parallel attachment model; text paste must keep its native behavior. |
