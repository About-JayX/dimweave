...
| Task 5 | `343ae415`, `e2500da4`, `977c8907`, `87d8a469`, `482e21fd` | Added persisted backend/frontend task-agent CRUD and reorder plumbing, emitted `task_agents_changed` context events, rebuilt the task pane around a concrete agent list, introduced add/edit agent UI plus drag reorder, converted `New Task`/`Edit Task` to an agent-array model, allowed empty-task creation, preserved existing `agentId`/`displayName` on edit, removed legacy singleton fallback badges from the header, and added interaction-level coverage for empty-task create, add/edit submit behavior, and drag reorder persistence using a test-only `happy-dom` harness. | `cargo check --manifest-path src-tauri/Cargo.toml --tests` ✅; `cargo test --manifest-path src-tauri/Cargo.toml task_graph:: -- --nocapture` ✅ 53 passed; `bun test tests/task-store.test.ts` ✅ 52 passed; `bun test src/components/ShellContextBar.test.tsx src/components/TaskContextPopover.test.tsx` ✅ 10 passed; `bun test src/components/TaskPanel/TaskHeader.test.tsx src/components/TaskPanel/TaskSetupDialog.test.tsx src/components/TaskPanel/TaskAgentList.test.tsx src/components/TaskPanel/TaskAgentEditor.test.tsx` ✅ 41 passed; `bun test src/components/ClaudePanel/connect-state.test.ts src/components/ClaudePanel/launch-request.test.ts src/components/AgentStatus/codex-launch-config.test.ts` ✅ 7 passed; `bun test src/components/ReplyInput/index.test.tsx` ✅ 5 passed; `bun run build` ✅; `git diff --check` ✅ | accepted |
| Task 6 | `5c663dfd` | Marked old singleton slot docs as fully superseded, added a stage-complete note to the replacement spec, updated CM records with accepted commit hashes and verification evidence, and added final integration regression tests confirming empty-task creation, dynamic role targets, broadcast semantics, no-task stability, and preserved UI error-log behavior across task-store, ReplyInput, ErrorBoundary, and ErrorLogDialog. | `cargo test --manifest-path src-tauri/Cargo.toml task_graph:: -- --nocapture` ✅ 53 passed; `bun test tests/task-store.test.ts src/components/ReplyInput/index.test.tsx src/components/ErrorBoundary.test.tsx src/components/ErrorLogDialog.test.tsx` ✅ 72 passed; `bun run build` ✅; `git diff --check` ✅ | accepted |
